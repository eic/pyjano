...
def print_first_time_message():
    mprint("""
The database file doesn't exist. Probably you run 'janapy' for one of the first times.

1. Install or check OS maintained required packages:
    > janapy req ubuntu         # for all packets janapy knows to built/install
    > janapy req usubnu ejana   # for ejana and its dependencies only
...
1. Set <b><blue>top-dir</blue></b> to start. This is where all missing packets will be installed.   
   ...
   > janapy --top-dir=<where-to-install-all>
   
2. You may have CERN.ROOT installed (req. version >= 6.14.00). Run this:

   > janapy set root `$ROOTSYS`
   
   You may set paths for other installed dependencies:
   > janapy install ejana --missing --explain    # to see missing dependencies
   > janapy set <name> <path>                    # to set dependency path
   
3. Then you can install all missing dependencies:

   > janapy install ejana --missing
